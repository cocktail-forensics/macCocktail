...
from scripts.macCocktail_functions import logfunc
from scripts.version_info import macCocktail_version, macCocktail_contributors
...
def get_icon_name(category, artifact):
    ...
    if category == 'SYSTEM OVERVIEW':       icon = 'git-commit'
    elif category == 'ACCOUNTS':      icon = 'user'
    elif category == 'NETWORK':     icon = 'wifi'
    elif category == 'CONNECTIONS':     icon = 'zap'
    elif category == 'RECENT ACTIVITY': icon = 'activity'
    elif category == 'DOWNLOADS': icon = 'download'
    elif category == 'SPOTLIGHT': icon = 'search'
    elif category == 'APPLICATIONS': icon = 'package'
    elif category == 'AUTORUNS': icon = 'activity'
...
def create_index_html(reportfolderbase, time_in_secs, time_HMS, extraction_type, image_input_path, nav_list_data):
    ...
    case_list = [   ['Processed Mount Point', image_input_path],
    ...
    authors_data = ''
    for author in macCocktail_contributors:
    ...
    filename = 'index.html'
    page_title = 'macCocktail Report'
    body_heading = 'macOS Forensic Artifact Parser'
    body_description = 'macCocktail is a collection of parsers for gathering forensic artifacts from macOS file systems'
    active_nav_list_data = mark_item_active(nav_list_data, filename)

    f = open(os.path.join(reportfolderbase, filename), 'w', encoding='utf8')
    f.write(page_header.format(page_title))
    f.write(body_start.format(f"macCocktail {macCocktail_version}"))

...
class ArtifactHtmlReport:
    ...
    def start_artifact_report(self, report_folder, artifact_file_name, artifact_description=''):
        ...
        self.report_file.write(page_header.format(f'macCocktail - {self.artifact_name} report'))
        self.report_file.write(body_start.format(f'macCocktail {macCocktail_version}'))
    ...
    def write_artifact_data_table(self, data_headers, data_list, source_path, 
        ...
        if write_location:
            self.write_lead_text(f'{self.artifact_name} parsed from: {source_path}')
